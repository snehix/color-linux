...
static int enable_regulator_pgood(struct regulator_dev *rdev)
{
	...
	printk("--------> enable_regulator_pgood 1\n");

	ret = regulator_enable_regmap(rdev);
	if (ret)
		return ret;
	
	printk("--------> enable_regulator_pgood 2\n");
	...
	printk("--------> enable_regulator_pgood 3\n");
	...
	printk("--------> enable_regulator_pgood 4\n");
...
static irqreturn_t sy7636a_pgood_irq_handler(int irq, void *_sy7636a)
{
	...
	printk("Power good line changed to %d\n", pwr_good);
...
static int sy7636a_regulator_resume(struct device *dev)
{
	...
	if (!sy7636a->vcom || sy7636a->vcom > 5000) {
		...
		printk("Vcom value invalid, and thus not restored\n");
	}
	else {
...
static struct gpio_desc* gpio_pmic_wakeup;
static struct gpio_desc* gpio_pmic_powerup;
...
static int sy7636a_regulator_probe(struct platform_device *pdev)
{
	...
	if (!sy7636a){
		dev_err(sy7636a->dev, "Unable to get driver data, EPROBE_DEFER \n");
		return -EPROBE_DEFER;
	}
...
#if 1
	gdp = devm_gpiod_get(sy7636a->dev, "epd-pmic-wakeup", GPIOD_OUT_HIGH);
	if (IS_ERR(gdp)) {
		dev_err(sy7636a->dev, "Wakeup GPIO fault %ld\n", PTR_ERR(gdp));
		return PTR_ERR(gdp);
	}
	
	gpio_pmic_wakeup = gdp;

	ret = gpiod_direction_output(gpio_pmic_wakeup,1);
	if (ret < 0)
		return ret;

	gdp = devm_gpiod_get(sy7636a->dev, "epd-pmic-powerup", GPIOD_OUT_HIGH);
	if (IS_ERR(gdp)) {
		dev_err(sy7636a->dev, "Powerup GPIO fault %ld\n", PTR_ERR(gdp));
		return PTR_ERR(gdp);
	}
	
	gpio_pmic_powerup = gdp;
	
	ret = gpiod_direction_output(gpio_pmic_powerup,1);
	if (ret < 0)
		return ret;
#endif
